...
#define toSizeT(t)    SIZE_T*(t/SIZE_T) + (t%SIZE_T ? SIZE_T : 0)  // TODO
//
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
		size_t t_prom = toSizeT(t);
		if(t_prom > getRemainSize())
			return false;
		ret = (T*)_ptr;
		_ptr = _ptr + t_prom;
      return true;
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      // TODO
		...
		{
			MemBlock<T>* destroy = _activeBlock ; 
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      // TODO
		...
		assert(tmp->_nextList == 0);
		MemRecycleList<T>* last = new MemRecycleList<T> (n);
		tmp->setNextList(last);
		return last;
   }
   ...
   T* getMem(size_t t) {
      ...
      //    => "ret" is the return address
		size_t n = getArraySize(t_prom);
		MemRecycleList<T>* check = getMemRecycleList(n);
		if(check->_first != 0)
		{
			ret = check->popFront();
			#ifdef MEM_DEBUG
      ...
      // 4. Get the memory from _activeBlock
		...
		if(_activeBlock->getMem(t_prom,ret))
		{
			#ifdef MEM_DEBUG
		...
		else
		{
			...
			_activeBlock = new MemBlock<T>(_activeBlock, _blockSize);	
      	    ...
      	    #endif // MEM_DEBUG
			_activeBlock->getMem(t_prom,ret);
			#ifdef MEM_DEBUG
          	cout << "Memory acquired... " << ret << endl;
          	#endif // MEM_DEBUG
   ...
   size_t getNumBlocks() const {
      // TODO
		size_t num = 1;
		MemBlock<T>* current = _activeBlock;
		while(current -> _nextBlock)
		{
			current = current -> _nextBlock;
			num ++;
		}
      return num;
   }
