...
DBAppendCmd::exec(const string& option)
{
   ...
   if (myStrNCmp("-Row", options[0], 2) == 0) doRow = true;
      ...
      return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);
   	...
   	int c;
	vector<int>_add;
	size_t _dataLength = doRow ? dbtbl.nCols() : dbtbl.nRows();
	for(size_t i = 1;  i <=  _dataLength; i++)
	{
		if(i >= options.size())
			_add.push_back(INT_MAX);
		else if(options[i] == "-" )
			_add.push_back(INT_MAX);
		else if (!(myStr2Int(options[i],c)))
		{
			cerr << "Error: " << options[i] << " is not a number!!" << endl;
...
{  
   // TODO...
	...
	else if(myStrNCmp("-Row",options[0],2) == 0)
	{
		...
		else if(!(checkRowIdx(options[1], rowIdx)))
			return CMD_EXEC_ERROR;		
		else
			cout << dbtbl[rowIdx];
	}
	else if(myStrNCmp("-Column",options[0],2) == 0)
	{
		if(options.size() == 1)
			return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
		else if (!(checkColIdx(options[1],colIdx)))
			return CMD_EXEC_ERROR;
		else
    ...
    else
	{
		if(options.size() == 1)
			return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
		else if(checkRowIdx(options[0], rowIdx) && checkColIdx(options[1], colIdx))
		{
